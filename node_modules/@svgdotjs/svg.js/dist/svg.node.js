...
function getOrigin(o, element) {
  ...
  if (typeof origin === 'string' || origin == null) {
    ...
    const bx = string.includes('left') ? x : string.includes('right') ? x + width : x + width / 2;
    const by = string.includes('top') ? y : string.includes('bottom') ? y + height : y + height / 2; // Set the bounds eg : "bottom-left", "Top right", "middle" etc...
...
const reference = /(#[a-z0-9\-_]+)/i; // splits a transformation chain

const transforms = /\)\s*,?\s*/; // Whitespace

const whitespace = /\s/g; // Test hex value

const isHex = /^#[a-f0-9]{3,6}$/i; // Test rgb value

const isRgb = /^rgb\(/; // Test css declaration

const isCss = /[^:]+:[^;]+;?/; // Test for blank string

const isBlank = /^(\s+)?$/; // Test for numeric string

const isNumber = /^[+-]?(\d+(\.\d*)?|\.\d+)(e[+-]?\d+)?$/i; // Test for percent value

const isPercent = /^-?[\d.]+%$/; // Test for image url

const isImage = /\.(jpg|jpeg|png|gif|svg)(\?[^=]+.*)?/i; // split at whitespace and comma

const delimiter = /[\s,]+/; // The following regex are used to parse the d attribute of a path
// Matches all hyphens which are not after an exponent

const hyphen = /([^e])-/gi; // Replaces and tests for all path letters

const pathLetters = /[MLHVCSQTAZ]/gi; // yes we need this one, too

const isPathLetter = /[MLHVCSQTAZ]/i; // matches 0.154.23.45

const numbersWithDots = /((\d?\.\d+(?:e[+-]?\d+)?)((?:\.\d+(?:e[+-]?\d+)?)+))+/gi; // matches .
...
class Symbol$2 extends Container {
  // Initialize node
  constructor(node) {
